...
/* Thin wrappers that inject the s_patch's current H into the generalized handles. */
...
void s_patch_init(s_patch_t *s_patch, M_p_general_t M_p_general, J_general_t J_general, double h, double eps_xi_eta, double eps_xy, MKL_INT n_xi, MKL_INT d, rd_mat_t *f_XY) {
    ...
    /* Wire the generalized handles through thin wrappers that capture H from s_patch. */
    q_patch_init(&(s_patch->Q),
        (M_p_t) {(M_p_handle_t) s_M_p_wrapper_function, (void*) s_patch},
        (J_t)   {(J_handle_t)   s_J_wrapper_function,   (void*) s_patch},
        eps_xi_eta, eps_xy, n_xi, d, 0.0, 1.0, 0.0, (d-1)*h, f_XY);
}
